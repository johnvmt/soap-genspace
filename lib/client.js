...
var Client = function(wsdl, endpoint, options) {
  ...
  try {
	  this['wsdl']['definitions']['xmlns'][targetNSAlias] = this['wsdl']['definitions']['xmlns']['tns'];
  }
  catch(err) {}
};
...
//    console.log("FN", typeof callback === "function");
...
Client.prototype._invoke = function(method, args, location, callback, options, extraHeaders) {
  ...
  message = args;
    ...
    message = self.wsdl.objectToDocumentXML(input.$name, args, input.targetNSAlias, input.targetNamespace, (input.$type || input.$lookupType)); /* THIS ONE */
  }
...
*/
	...
	var envelopeStr = "";
	for(var key in this['wsdl']['definitions']['xmlns'])
		envelopeStr = "xmlns:" + key + "=\"" + this['wsdl']['definitions']['xmlns'][key] + "\" ";
  ...
  var respCallback = function(err, response, body) {
    var result;
    var obj;
  ...
  var respCallbackNoResponse = function(error, response) {
  	
  };
  
  if(typeof callback != "function")
  	req = http.requestNoResponse(location, xml, headers, options);
  else
	req = http.request(location, xml, respCallback, headers, options);
